...
def validate_blockchain(state: Optional[BlockchainState]) -> ValidatorError:
    """
    Spiegel der C-Funktion eltt_validator_check_blockchain.
    Führt KEINE Mutationen aus, prüft nur:
    - Struktur
    - Token
    - Wallets
    - Pools
    - Stakes
    - Blöcke / Hashes / prev_hash / Timestamps / TXs
    """
    if state is None:
    ...
    # Wallets prüfen (Form, keine negativen Balances)
    for w in state.wallets:
        if not w.address or len(w.address) >= ELTT_MAX_ADDRESS_LEN:
    ...
    for p in state.pools:
        if p.token_x_index < 0 or p.token_x_index >= len(state.token_types):
            return ValidatorError.POOL_INDEX_INVALID
        if p.token_y_index < 0 or p.token_y_index >= len(state.token_types):
            return ValidatorError.POOL_INDEX_INVALID
        if p.lp_token_index < 0 or p.lp_token_index >= len(state.token_types):
            return ValidatorError.POOL_INDEX_INVALID
    ...
    for s in state.stakes:
        if not s.owner or len(s.owner) >= ELTT_MAX_ADDRESS_LEN:
            return ValidatorError.STAKE_OWNER_INVALID
        if s.token_index < 0 or s.token_index >= len(state.token_types):
            return ValidatorError.TX_TOKEN_INDEX_INVALID
        if s.amount < 0.0:
    ...
    # Block- und TX-Prüfungen (ohne Hash-Neuberechnung, da C-Referenz maßgeblich ist)
    last_ts = 0
    seen_tx_hashes_per_block: List[int] = []

    for i, blk in enumerate(state.blocks):
        if i == 0:
            # Genesis: prev_hash muss 32x 0 sein
        ...
        seen_tx_hashes_per_block.clear()
        for tx in blk.txs:
            ...
            if not tx.from_address or len(tx.from_address) >= ELTT_MAX_ADDRESS_LEN:
                return ValidatorError.WALLET_ADDRESS_INVALID
            if not tx.to_address or len(tx.to_address) >= ELTT_MAX_ADDRESS_LEN:
                return ValidatorError.WALLET_ADDRESS_INVALID

            # einfache Replay-Erkennung (Python-Spiegel, nicht sicherheitskritisch)
            h = hash(
                (
                    tx.from_address,
                    tx.to_address,
            ...
            if h in seen_tx_hashes_per_block:
                return ValidatorError.TX_REPLAY_DUPLICATE_IN_BLOCK
            seen_tx_hashes_per_block.append(h)
